...
public class PlaneColaboratorPilotPlaneInteractionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public PlaneColaboratorPilotPlaneInteractionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      // Next states after receiving "InteractionUnit2"
			       ...
			       smf.add("waiting for InteractionUnit2","InteractionUnit3");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("InteractionUnit3", "endInteractionUnit3");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){     
     ...
     CommActCreator.generateRWithoutCID((JADEAgent)myAgent,
      "PilotPlaneInteraction","enable",this.getPlayedRole(),options,this);
  ...
  if (this.isState("InteractionUnit3")) {
     ...
     try {
      ...
      String[] optionsA=new String[options.size()];
      options.toArray(optionsA);
      if (this.getDCC().notifyMessageSent("InteractionUnit3",optionsA,this)){
           ...
           "InteractionUnit3","PilotPlaneInteraction",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("InteractionUnit3", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("InteractionUnit3", "ABORTED");
    	  } else  {
