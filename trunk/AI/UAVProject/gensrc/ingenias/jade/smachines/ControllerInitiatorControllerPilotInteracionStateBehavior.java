...
public class ControllerInitiatorControllerPilotInteracionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public ControllerInitiatorControllerPilotInteracionStateBehavior( String agentName,
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","ControllerPilotInteracion",completelistInvolvedActors);
    ...
    //in the parent class
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                   ...
                   "ControllerPilotInteracion",this.getPlayedRole(),
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InteractionUnit0",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "InteractionUnit0","ControllerPilotInteracion",this.getContentForNextMessage());
           getTimeout().stop();
