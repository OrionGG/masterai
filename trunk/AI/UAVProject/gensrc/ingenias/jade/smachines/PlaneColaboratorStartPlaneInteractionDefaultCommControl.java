...
import ingenias.exception.NotFound;
  ...
  public class PlaneColaboratorStartPlaneInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public PlaneColaboratorStartPlaneInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("PlaneOn");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit7") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"PlaneOn").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit7");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"PlaneOn");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("PlaneOn");
	       lockProcessed("PlaneOn");
           //MainInteractionManager.log("Removing lock PlaneOn",this.getAgent().getLocalName()+"-"+sb.getConversationID());
