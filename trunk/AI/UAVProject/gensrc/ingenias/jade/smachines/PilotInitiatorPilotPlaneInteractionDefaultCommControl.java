...
import ingenias.exception.NotFound;
  ...
  public class PilotInitiatorPilotPlaneInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public PilotInitiatorPilotPlaneInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Manoeuvre");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit2") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Manoeuvre").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit2");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Manoeuvre");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Manoeuvre");
	       lockProcessed("Manoeuvre");
           //MainInteractionManager.log("Removing lock Manoeuvre",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("InteractionUnit3");
...
public boolean continueProcess(Vector<ACLMessage> multipleMessages,String[] options,
  ...
  // String sequence= sb.getState();
    ...
    if (sb.isState("waiting for InteractionUnit3")&& options.length>0 && mes!=null
    && mes.getUserDefinedParameter("sequence")!=null &&
    		mes.getUserDefinedParameter("sequence").equals("InteractionUnit3")){
    	 boolean allexist=true;
         
         if (allexist && true){
     	   sb.removeState("waiting for InteractionUnit3");           
    	   //try {
		   ...
		   futureStates.add("endInteractionUnit3");
