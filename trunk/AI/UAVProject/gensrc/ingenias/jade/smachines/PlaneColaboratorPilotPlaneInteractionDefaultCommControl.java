...
import ingenias.exception.NotFound;
  ...
  public class PlaneColaboratorPilotPlaneInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public PlaneColaboratorPilotPlaneInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Plane_Change");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit3") &&
    	 sb.isState("InteractionUnit3")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Plane_Change").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("InteractionUnit3");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Plane_Change");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Plane_Change");
	       lockProcessed("Plane_Change");
           //MainInteractionManager.log("Removing lock Plane_Change",this.getAgent().getLocalName()+"-"+sb.getConversationID());
