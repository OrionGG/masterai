...
import ingenias.exception.NotFound;
  ...
  public class ControllerInitiatorControllerPilotInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public ControllerInitiatorControllerPilotInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("Order");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit0") &&
		 ...
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Order").isEmpty();
         ...
         if (allexist && true){
           ...
           sb.removeState("InteractionUnit0");
	   	   ...
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"Order");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("Order");
	       lockProcessed("Order");
           //MainInteractionManager.log("Removing lock Order",this.getAgent().getLocalName()+"-"+sb.getConversationID());
