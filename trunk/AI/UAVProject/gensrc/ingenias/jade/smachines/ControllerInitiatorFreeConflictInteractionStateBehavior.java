...
public class ControllerInitiatorFreeConflictInteractionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public ControllerInitiatorFreeConflictInteractionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "InteractionUnit8");
			    
			    
			    
			     // States involved in message deliver
			     
			      smf.add("InteractionUnit8", "endInteractionUnit8");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","FreeConflictInteraction",completelistInvolvedActors);
  ...
  if (this.isState("InteractionUnit8")) {
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InteractionUnit8",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "InteractionUnit8","FreeConflictInteraction",this.getContentForNextMessage());
           getTimeout().stop();
            this.notifyStateTransitionExecuted("InteractionUnit8", options.firstElement().toString());
      } else {
    	  if (getTimeout().isStarted() && getTimeout().isFinished()){
    	    		 this.abortDueTimeout();   	        
    	    		  this.notifyStateTransitionExecuted("InteractionUnit8", "ABORTED");
    	  } else  {
