...
public class ControllerInitiatorControllerPilotInteractionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public ControllerInitiatorControllerPilotInteractionStateBehavior( String agentName,
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","ControllerPilotInteraction",completelistInvolvedActors);
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InteractionUnit0",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "InteractionUnit0","ControllerPilotInteraction",this.getContentForNextMessage());
           getTimeout().stop();
