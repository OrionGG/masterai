...
public class Simulation_CreatorJADEAgent
		 extends JADEAgent {         
 
		 public Simulation_CreatorJADEAgent(){
		 super(new Simulation_CreatorProtocol(),new Simulation_CreatorInteractionLocks());
		 }
   ...
   /* */
	...
	Vector evetns=null;
     ...
     app=EnvironmentInit.createInstance(this);
	 //app.registerOwner(this);
	 	    
     this.getAM().addApplication("Environment",app);        
	 events=new Vector();
	 actions=new Vector();

	 if (getGraphics()!=null)
	  getGraphics().addApplication("Environment", events,actions);    
