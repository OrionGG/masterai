...
public class PilotInitiatorPilotPlaneInteractionStateBehavior extends StateBehavior{
private MentalStateReader msr=null;
  public PilotInitiatorPilotPlaneInteractionStateBehavior( String agentName,
    ...
    try {
			if (IAFProperties.getGraphicsOn()){
			      ...
			      smf.add("disabled", "InteractionUnit2");
			    
			    
			      // Receiving a message    
			      smf.add("InteractionUnit3", "waiting for InteractionUnit3");
			      
			      // Next states after receiving "InteractionUnit3"
			      
			       smf.add("waiting for InteractionUnit3","endInteractionUnit3");
			      
			    
			    
			     // States involved in message deliver
			     
			      smf.add("InteractionUnit2", "InteractionUnit3");
  ...
  public synchronized void action() {
    ...
    if (this.isState("disabled")){
      ...
      CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
       "enable","PilotPlaneInteraction",completelistInvolvedActors);
  ...
  if (this.isState("InteractionUnit3") ) {  // State changed by other agent and upted
    //in the parent class
         
      Vector options=new Vector();
      
      options.add("endInteractionUnit3");
      ...
      if (allexist && true){     
		 addCreatedBehaviors(CommActCreator.generateR((JADEAgent)myAgent,
                        this.getConversationID(),"InteractionUnit3",
                   "PilotPlaneInteraction",this.getPlayedRole(),
                   optionsA,this,cardinality,0));
      }	 
      this.removeState("InteractionUnit3");
      this.addState("waiting for InteractionUnit3");
      this.notifyStateTransitionExecuted("InteractionUnit3", "waiting for InteractionUnit3");
  } 
     ...
     try {
      ...
      if (this.getDCC().notifyMessageSent("InteractionUnit2",optionsA,this)){
           //If mental state conditions are met, the message is send and state changed
            CommActCreator.generateSObject((JADEAgent)myAgent,rolesv,actors,this.getConversationID(),
           "InteractionUnit2","PilotPlaneInteraction",this.getContentForNextMessage());
           getTimeout().stop();
  ...
  if (this.isState("endInteractionUnit3")) {
