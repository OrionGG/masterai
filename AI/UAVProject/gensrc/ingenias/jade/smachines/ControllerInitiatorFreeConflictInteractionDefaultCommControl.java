...
import ingenias.exception.NotFound;
  ...
  public class ControllerInitiatorFreeConflictInteractionDefaultCommControl extends DefaultCommControl{
  ...
  public ControllerInitiatorFreeConflictInteractionDefaultCommControl(String cid, MentalStateReader msr, ingenias.jade.comm.LocksRemover lr){
  super(msr, lr);
  ...
  public static void addDefaultLocks(LocksWriter lw){
    ...
    // Facts that cannot be removed because they must be sent
     ...
     lw.addDeletionLockExpectedType("ConflictFinished");
 ...
 public boolean continueProcessSend(String stateToEvaluate,String[] options,
   ...
   boolean processed = false;
    ...
    if (stateToEvaluate.equals("InteractionUnit8") &&
    	 sb.isState("InteractionUnit8")&& options.length>0) {         
         boolean allexist=true;
         Vector<MentalEntity> mfcontent=null;
         
		 
		 allexist=allexist && !getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ConflictFinished").isEmpty();
		   
         if (allexist && true){
           sb.clearContentNextMessage();
           sb.removeState("InteractionUnit8");
           
           
	   	   mfcontent=getMSR().obtainConversationalMentalEntityByType(sb.getConversation(),"ConflictFinished");
	   	   for (MentalEntity me:mfcontent)             
             sb.addContentForNextMessage(me);    
	       getLR().removeDeletionLockType("ConflictFinished");
	       lockProcessed("ConflictFinished");
           //MainInteractionManager.log("Removing lock ConflictFinished",this.getAgent().getLocalName()+"-"+sb.getConversationID());
            
             
           
  		   //sb.clearState();         
	              
		   futureStates.add("endInteractionUnit8");
