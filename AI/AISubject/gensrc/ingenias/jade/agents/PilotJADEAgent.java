...
public class PilotJADEAgent
	...
	private boolean initialiseNonConversationalTask(Task tobject) {
	   ...
	   Vector<String> repeatedOutputs=new Vector<String>();
		...
		expectedInput=this.getMSM().getMentalEntityByType("Head_Change");
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
				nonExistingInputs.add("Head_Change");
			 } else {
			    addExpectedInputs(tobject, "Head_Change","1",expectedInput);
             	addConsumedInput(to,"Head_Change",expectedInput);
			 }
             allEntitiesExist=allEntitiesExist || expectedInput.size()!=0;
		} 
		...
		if (this.getLM().canBeDeleted(expectedInput)){             
         ...
         if (tobject.getType().equals("AvoidCollisionTask") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
	 ...
	 /*    
	      ...
	      */	  
	      ...
	      /**/	      
		    ...
		    {Head_Change expectedOutputHead_Change=		    
		     new Head_Change(MentalStateManager.generateMentalEntityID());			
             to.add(new OutputEntity(expectedOutputHead_Change,TaskOperations.CreateWF));
            }
		...
		return initialised;	       
         ...
         nonExistingInputs.clear();
  		 repeatedOutputs.clear();
         if (tobject.getType().equals("ExecutePlan") ){
            Vector<MentalEntity> expectedInput=null;
            RuntimeFact expectedOutput=null;
            RuntimeEvent expectedOutputEvent=null;
			RuntimeConversation expectedInt=null;
	...
	private boolean initialiseConversationalTask(RuntimeConversation conversation, Task tobject) {
	   ...
	   if (tobject.getType().equals("DeleteNonUsedEntitiesTask")){
		...
		expectedInput=this.getMSM().obtainConversationalMentalEntityByType(conversation,"Head_Change");
		if (this.getLM().canBeDeleted(expectedInput)){                          
             if (expectedInput.size()==0){
				nonExistingInputs.add("Head_Change");
			 } else {
	...
	public Vector tasksThatSatisfyGoal(String goalname){
		 ...
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new AvoidCollisionTaskTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
         ...
         if (goalname.equals("ArrivedOnTime")){
         
         {
         boolean canbescheduled=false;
		 Task tobject=null;		 
				// If a conversational initialization fails, a conventional one is tried
				 tobject=new ExecutePlanTask(ingenias.jade.MentalStateManager.generateMentalEntityID());
				 canbescheduled=initialiseNonConversationalTask(tobject);
			 	 if (canbescheduled){
					//MainInteractionManager.log("Scheduled task "+tobject.getType()+" to achieve goal ArrivedOnTime",getLocalName()+"-"+tobject.getType());
					tasks.add(tobject);
				 } 			
	     }
	...
	public void setup() {
		...
		Vector<String> ttypes=new Vector<String>(); 
         ...
         ttypes.add("AvoidCollisionTask");		         
                
         
         
         ttypes.add("ExecutePlan");		         
   ...
   sg= new ingenias.editor.entities.StateGoal("ArrivedOnTime");
   sg.setState("pending");
      try {
	   this.getMSM().addMentalEntity(sg);
   } catch (InvalidEntity e1) {
   ...
   java.awt.event.ActionListener ifPressed=null;
     ...
     final JADEAgent _jaInitiatorPilotPlaneInteraction=this;
     ifPressed=new java.awt.event.ActionListener() {
       public void actionPerformed(ActionEvent e) {
         _jaInitiatorPilotPlaneInteraction.addBehaviour(
           ...
           public void action() {
			 ...
			 if (allexist){
              ...
              if (getGraphics()!=null)
                getGraphics().runtimeWarning(" Mental conditions required for starting InitiatorPilotPlaneInteraction "+
     ...
     if (getGraphics()!=null)
      getGraphics().addInteraction(this.getName(), "Role: Initiator - Int: PilotPlaneInteraction", ifPressed);
     getCM().addInitiatorRoles("PilotPlaneInteraction","Initiator");
