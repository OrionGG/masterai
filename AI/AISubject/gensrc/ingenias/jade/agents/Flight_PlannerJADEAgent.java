...
public class Flight_PlannerJADEAgent
		 extends JADEAgent {         
 
		 public Flight_PlannerJADEAgent(){
		 super(new Flight_PlannerProtocol(),new Flight_PlannerInteractionLocks());
		 }
		...
		if (this.getLM().canBeDeleted(expectedInput)){             
             if (expectedInput.size()==0){
			    ...
			    addExpectedInputs(tobject, "Flight_Plan","1",expectedInput);
             	addConsumedInput(to,"Flight_Plan",expectedInput);
			 }
   ...
   ingenias.jade.components.Application app=null;	  
	...
	Vector evetns=null;
     ...
     this.getAM().addApplication("Plan_Selector",app);        
	 events=new Vector();
	 actions=new Vector();

	 if (getGraphics()!=null)
	  getGraphics().addApplication("Plan_Selector", events,actions);    
